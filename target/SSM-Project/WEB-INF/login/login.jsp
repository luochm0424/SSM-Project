...
<body class="login-bg">
    ...
    <div class="layui-form-item" pane=""></div>
      ...
      <input type="checkbox" lay-filter="filter" checked="checked"  id="memoryuser" lay-skin="primary" title="30天内自动登录" >
	...
	<script>
		...
		$(function () {
			$.ajax({
				type:"post",
				url:"/loginController/logout.ajax",
				success:function (info) {
					$("#username").val(info.username);
					$("#password").val(info.password);
					if(info==null){
						$("#memoryuser").checked(false)
					}
				},
				error:function () {
		...
		function login() {
			...
			$.ajax({
				type:"post",
				url:"/loginController/checkLogin.ajax?remember="+remember,
				dataType:"text",
				data:$("form").serialize(),
				success:function (info) {
					if(info=="success"){
						window.location.href="/loginController/index.do"
					}else{
